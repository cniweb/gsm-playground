...
void GSM::DebugPrint(const char *string_to_print, byte last_debug_print)
{
  if (last_debug_print) {
    ...
    SendATCmdWaitResp("AT", 500, 20, "OK", 1);
  }
...
char GSM::SendSMS(char *number_str, char *message_str) 
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    // 20 msec. for inter character timeout
    if (RX_FINISHED_STR_RECV == WaitResp(1000, 20, ">")) {
      // send SMS text
      ...
      if (RX_FINISHED_STR_RECV == WaitResp(7000, 20, "OK")) {
#else 
      Serial.print(0x1a, BYTE);
      if (RX_FINISHED_STR_RECV == WaitResp(7000, 20, "+CMGS")) {
#endif
...
char GSM::InitSMSMemory(void) 
{
  ...
  SendATCmdWaitResp("AT+CNMI=2,0", 1000, 20, "OK", 2);
